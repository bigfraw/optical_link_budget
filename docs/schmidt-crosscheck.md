...
| `_merge_layers` / `_plan_space_continuous` | `olb/waveoptics/turbulence/sampling.py` | where the screens go, and what each carries | (9.65) | 164 | 177 | partial | The DEFAULT planner is now CONTINUOUS (`_plan_space_continuous`, item 2-I2 step 1, 2026-09-02): it integrates the Cn2 callable and cuts the path into equal-Rytov-weight slabs with a Cn2-weighted centroid per slab. `_merge_layers` is the LEGACY array fallback (an explicit hs grid). Neither SOLVES Eq. (9.65), but both hold all 8 moments of the default profile inside 1 percent; the module self-check measures that for both. See Table 2, row S-22, and the WP7 note. |
...
| `min_screens` | `olb/waveoptics/turbulence/sampling.py:183` | the screen-count floor | — | — | — | olb rule | Chapter 9 gives NO such floor. WP7 kept 15 / 9 / 5 and it re-sourced them to an olb convergence sweep, with the moment floor of 4 as the absolute lower bound. See Table 3 and the WP7 note. |
...
| S-22 | 9.2.5 | (9.65) | The layered-atmosphere MOMENT rule for `0 <= m <= 7`. It is the only screen-placement rule that Chapter 9 gives. Built as `profile_moments`, `layer_moments`, `moment_error`. WP7 MEASURED it against the production grouping: the error stays inside 1 percent for every moment. The 2-I2 step-1 CONTINUOUS planner (`_plan_space_continuous`, 2026-09-02) places screens by equal Rytov weight with a Cn2-weighted centroid, and its moment error is also measured inside 1 percent in the self-check. The planner still does not SOLVE the rule, and `SamplingReport` does not carry the error. Gauss-quadrature placement is now implementable on the continuous base. | `olb/waveoptics/turbulence/sampling.py` | low (WP7 measured it) |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `QualityPreset.min_screens` | 15 / 9 / 5 | `olb/waveoptics/turbulence/sampling.py:183` | a screen-count FLOOR | **the book would not give** | — | — | — | checked, no book source. WP7 sourced it to an olb convergence sweep, and it kept the values. See the WP7 note. |
| `QualityPreset.sigma2_r_screen_max` | 0.2 / 0.4 / 0.4 | `olb/waveoptics/turbulence/sampling.py:196` (PRESETS) | `rmax`, the per-screen cap | 0.1 on `sigma_chi^2`, so **0.4 on `sigma_R^2`** | Listing 9.5, lines 37, 38 | 175 | 188 | checked. Since 2026-09-06 (owner decision) `standard` and `rapid` EQUAL the book cap, and `reference` is 2x stricter. See the factor note below. |
| `QualityPreset.boundary_width_frac` | 0.125 / 0.125 / 0.10 | `olb/waveoptics/turbulence/sampling.py:196` | the boundary half-width | `0.47 N dx` (Listing 9.7); `0.45 L` (Fig. 8.1) | (8.1); Listing 9.7, line 19 | 134, 179 | 147, 192 | checked, the book gives no value in this parameterisation |
...
| `QualityPreset.guard` | 4 / 3 / 2 | `olb/waveoptics/turbulence/sampling.py:196` | the beam-radius margin | the book gives no such margin; it sizes from D1', D2' and constraint 2 | (9.84)–(9.87) | 173, 174 | 186, 187 | checked, different route |
| `QualityPreset.pixels_per_r0` | 4 / 3 / 2 | `olb/waveoptics/turbulence/sampling.py:196` | the phase pitch rule | **3.01** pixels per r0 | Sec. 9.4 text with (9.44) | 172, 160 | 185, 173 | checked, `standard` = 3 matches |
| `QualityPreset.fresnel_weight_min` | 0.005 / 0.02 / 0.05 | `olb/waveoptics/turbulence/sampling.py:196` | a weak-screen exemption | **the book would not give** | — | — | — | checked, no source. See gap S-25. |
...
The olb `_screen_rytov` (`sampling.py:288`) computes
