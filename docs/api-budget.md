...
- [`Term` (`olb/results.py`)](#term-olbresultspy)
  - [The three faces](#the-three-faces)
  - [`stochastic`](#stochastic)
  - [`quantile_db(p)` fallback rules](#quantile_dbp-fallback-rules)
  - [The mean-only Term](#the-mean-only-term)
- [`Budget` (`olb/results.py`)](#budget-olbresultspy)
  - [`Budget(terms=None, tx_power_dbm=None, rx_sensitivity_dbm=None, scenario=None)`](#budgettermsnone-tx_power_dbmnone-rx_sensitivity_dbmnone-scenarionone)
  - [`add(term)`](#addterm)
  - [`total_loss_db()`](#total_loss_db)
  - [`to_frame()`](#to_frame)
  - [`assumptions_frame()`](#assumptions_frame)
  - [`constraints_frame()`](#constraints_frame)
  - [`check(warn=True)`](#checkwarntrue)
  - [`mean_only_terms()`](#mean_only_terms)
  - [`provides_fade`](#provides_fade)
  - [`fade_margin_db(availability)`](#fade_margin_dbavailability)
  - [`monte_carlo(n, rng=None, availabilities=(0.99,))`](#monte_carlon-rngnone-availabilities099)
- [`Assumptions` (`olb/assumptions.py`)](#assumptions-olbassumptionspy)
  - [`Assumptions(beam_type, turbulence_regime, spectrum, validity="", violations=[], constraints=[], provenance=[])`](#assumptionsbeam_type-turbulence_regime-spectrum-validity-violations-constraints-provenance)
  - [`ok`](#ok)
  - [`flag(reason, source=None)`](#flagreason-sourcenone)
- [The function-owned assumption mechanism (`olb/assumptions.py`)](#the-function-owned-assumption-mechanism-olbassumptionspy)
  - [`Constraint(kind, statement, doi, where="", check=None)`](#constraintkind-statement-doi-where-checknone)
  - [`@assumes(*constraints, beam_type=..., turbulence_regime=..., spectrum=...)`](#assumesconstraints-beam_type-turbulence_regime-spectrum)
  - [`trace_assumptions()`](#trace_assumptions)
  - [`merge_assumptions(*records, validity="")`](#merge_assumptionsrecords-validity)
- [`units` (`olb/units.py`)](#units-olbunitspy)
  - [`loss_db(transmission)`](#loss_dbtransmission)
  - [`combine_db(*terms_db)`](#combine_dbterms_db)
- [The four budget entry points](#the-four-budget-entry-points)
...
- [Several detectors: `multi_detector_budgets` (`olb/multidetector.py`)](#several-detectors-multi_detector_budgets-olbmultidetectorpy)
  - [`multi_detector_budgets(scenario, geometry, detectors, *, wave=None, **kwargs)`](#multi_detector_budgetsscenario-geometry-detectors--wavenone-kwargs)
- [The receive beamsplitter (`olb/models/splitter.py`)](#the-receive-beamsplitter-olbmodelssplitterpy)
  - [`resolve_fracs(detectors)`](#resolve_fracsdetectors)
  - [`splitter_term(frac, *, name=None, note=None)`](#splitter_termfrac--namenone-notenone)
  - [`arm_scenario(scenario, detector)`](#arm_scenarioscenario-detector)
- [An elevation sweep: `budgets_vs_elevation` (`olb/sweep.py`)](#an-elevation-sweep-budgets_vs_elevation-olbsweeppy)
  - [`budgets_vs_elevation(scenario, elevations, *, geometry_factory=None, **kwargs)`](#budgets_vs_elevationscenario-elevations--geometry_factorynone-kwargs)
