...
6. **UF-01 — `WEAK_FLUCTUATION_LIMIT = 0.6` warns 2.4 times too late.**
...
| UF-01 | WEAK_FLUCTUATION_LIMIT = 0.6 | olb/turbulence/uplink_flux.py:48 | validity ceiling on the log-amplitude variance sigma2_x | 8.2 | Ch. 8, text below Eq. (23) | 264-265 | 289-290 | weak | wrong | CONFIRMED BY 2 READERS. The book defines weak fluctuation by sigma_R^2 < 1 (Ch. 8 printed 264; Ch. 12, Eqs. (40) printed 497 and (93) printed 522). With sigma_I^2 = 4 sigma_x^2 (Ch. 8, Eq. (13)) the book limit is sigma_x^2 < 0.25. The code allows 0.6, that is sigma_R^2 = 2.4, so the warning fires 2.4 times too late. The 0.6 comes from the Dios split-step comparison, not from Andrews. The same 0.6 is in the `beam_wave_scintillation.py:25` docstring. |
...
| G-24 | 5.7.2 | Ch. 5, Eq. (93) | 156 | 181 | One shared lognormal irradiance PDF object with the three faces. olb writes the same four expressions inline in two files (DL-01 to DL-04 and TL-01 to TL-04). | distributions.py | 10 | P2 |
