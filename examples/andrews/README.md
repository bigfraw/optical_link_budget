...
| `scintillation_regimes.py` | The scintillation index from weak fluctuation to saturation for the plane wave, the spherical wave and the Gaussian beam, with the two regime boundaries marked; the inner-scale effect on the weak index; the strong two-scale index for the plane wave and the spherical wave, with the refusal of the Gaussian strong two-scale case (Ch. 9, Eq. (109) eta_X is unresolved); the tracked and the untracked Gaussian beam off axis. Ch. 8, Ch. 9, Ch. 12. |
...
| `temporal_statistics.py` | The temporal irradiance spectrum (weak, strong, and strong with an aperture), the quasi-frequency and its band dependence, the inner-scale bound on the quasi-frequency (the modified von Karman cutoff), the Greenwood frequency and tau0 across elevation, and the fade numbers they feed. Ch. 8, Ch. 9, Ch. 11, Ch. 14. |
