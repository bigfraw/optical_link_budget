...
Run from the repo root:
    python -m examples.uplink_divergence
'''
...
def main():
    base_link = Link(
        ...
        tx_power_dbm=42.0,          # ~16 W launch power
    ...
    theta_min = base_link.wavelength_m / (np.pi * base_link.tx_waist_m)
    print(f"Aperture w0 = {base_link.tx_waist_m} m  ->  diffraction-limited "
    ...
    for label, divergence in cases:
        scn = Scenario(link=replace(base_link, divergence_rad=divergence),
                       site=site, altitude_m=altitude_m)
